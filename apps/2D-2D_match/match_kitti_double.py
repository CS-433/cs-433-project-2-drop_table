...
parser.add_argument('--logdirbis', help='path to the log directory')
args = parser.parse_args()

logdir = args.logdir
logdirbis = args.logdirbis
...
fnamebis = os.path.join(logdirbis, 'model.pth')
print('> Loading model from {}....'.format(fnamebis))
...
modelbis = PatchNetAutoencoder(
    args['embedding_size'],
    args['normalize']
)
modelbis.load_state_dict(torch.load(fnamebis)['patchnet'])
modelbis.to(device)
modelbis.eval()
...
# color1 = cv.imread('samples/EPFL_2020-09-17-piloted_0_DJI_0029_img.png')
color0 = cv.imread('samples/comballaz-air2_1_DJI_0001_f2_img.png')
color1 = cv.imread('samples/comballaz-air2_3_DJI_0002_f2_img.png')
...
desc0bis = compute_lcd_descriptors(patches0, modelbis, args['batch_size'], device)
desc1bis = compute_lcd_descriptors(patches1, modelbis, args['batch_size'], device)
...
bf2 = cv.BFMatcher()
matchesbis = bf2.match(desc0bis, desc1bis)
matchesbis = sorted(matchesbis, key=lambda x:x.distance)

image0 = cv.drawMatches(color0, keypts0, color1, keypts1, matches[500:550], None, singlePointColor=None, flags=2)
image1 = cv.drawMatches(color0, keypts0, color1, keypts1, matchesbis[500:550], None, singlePointColor=None, flags=2)
